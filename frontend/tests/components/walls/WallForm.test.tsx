...
describe('WallForm', () => {
  it('rejects zero width', async () => {
    render(<WallForm onSubmit={vi.fn()} />);
    ...
    fireEvent.submit(screen.getByRole('form', { name: /Wall form/i }));
  ...
  it('pre-populates in edit mode', () => {
    render(<WallForm initialValues={{ label: 'North', width: 3, height: 2 }} onSubmit={vi.fn()} />);
